...
- (CGRect)frameOfPageAtIndex:(NSInteger)pageIndex {
	...
	if (UIInterfaceOrientationIsLandscape(_orientation)) {
		if (size.width > size.height || size.width / size.height > aspectRatio) {
...
- (void)layoutPage {
	UIView* page;
	if(!(page = [self pageAtIndex:_centerPageIndex create:YES])) return;
	...
	if (self.zoomed) {
		...
		page.transform = [self rotateTransform:CGAffineTransformScale(CGAffineTransformMakeTranslation(offset.x, offset.y), zoom, zoom)];
		page.frame = CGRectMake(offset.x + frame.origin.x*zoom, offset.y + frame.origin.y*zoom, frame.size.width*zoom, frame.size.height*zoom);
	} else {
		page.transform = rotation;
		page.frame = CGRectMake(offset.x + frame.origin.x, offset.y + frame.origin.y, frame.size.width, frame.size.height);
	}
}
...
#pragma mark -
#pragma mark Responder/Touches methods
